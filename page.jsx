...
import { testimonials, accordionData } from "./src/app/data/fleetData";
import { fleetList } from "./src/app/data/CarsDetails";
...
import RentalCarForm from "./src/app/components/RentalCarForm";
import ServicePrice from "./src/app/components/ServicePrice";
...
function Hero() {
  return (
    <div className="relative min-h-[70vh]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-20 lg:pt-30 text-center lg:text-left">
        {/* Badge */}
        ...
        {/* Main Heading */}
        <h1 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold text-white leading-tight mb-6 drop-shadow-lg">
        ...
        <h2 className="text-2xl sm:text-3xl lg:text-4xl font-semibold text-white/90 mb-6">
          Affordable & Reliable Rentals
        </h2>

        {/* Description */}
        <p className="text-gray-300 max-w-4xl mx-auto lg:mx-0 mb-12 text-sm sm:text-md">
          ✅ Rideshare & delivery ready | ✅ Flexible weekly rentals | ✅ Local Sunbury support
        </p>

        {/* Buttons */}
        <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
          <a href="#ChooseVehicles">
            <button className="bg-white text-black px-8 py-3 rounded-full font-medium text-base sm:text-[14px] flex items-center gap-2 shadow-lg hover:scale-105 hover:shadow-xl transition-all duration-300">
              Browse Cars <ArrowRight className="mt-[2px]" size={18} />
            </button>
          </a>
          <a
            href="tel:+61430410450"
            className="border-2 border-white text-white px-8 py-3 rounded-full text-base sm:text-[14px] hover:bg-white hover:text-black transition-all duration-300 flex items-center justify-center shadow-sm"
          >
            Call Now
          </a>
        </div>
  ...
  return (
    <div className="border rounded-2xl overflow-hidden hover:shadow-lg transition">
      ...
      <div className="p-8">
        ...
        {/* Price (optional) */}
        {car.price && (
          <p className="text-xl font-bold mt-1 mb-2">{car.price}</p>
        )}

        {/* Details / Description */}
        {car.details && (
          <p className="text-sm text-gray-600 mb-1">{car.details}</p>
        )}
        {car.description && (
